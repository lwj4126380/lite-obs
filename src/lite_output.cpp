...
{
    std::shared_ptr<lite_obs_output_signals> signal_callback{};

    bool received_video{};
    bool received_audio{};
    std::atomic_bool data_active{};
    std::atomic_bool end_data_capture_thread_active{};
    ...
    std::atomic_bool reconnect_thread_active{};
...
void lite_obs_output::set_output_signal_callback(std::shared_ptr<lite_obs_output_signals> sig)
{
...
void lite_obs_output::lite_obs_output_destroy()
{
    ...
    if (d_ptr->end_data_capture_thread_active) {
...
bool lite_obs_output::lite_obs_output_actual_start()
{
    ...
    if (success && vo) {
        ...
        auto video = obs.obs_core_video();
        d_ptr->starting_drawn_count = video->total_frames();
        d_ptr->starting_lagged_count = video->lagged_frames();
    }
...
void lite_obs_output::obs_output_actual_stop(bool force, uint64_t ts)
{
    if (stopping() && !force)
        return;
    ...
    if (d_ptr->reconnecting) {
        ...
        if (d_ptr->reconnect_thread_active) {
...
void lite_obs_output::lite_obs_output_force_stop()
{
    if (!stopping()) {
        d_ptr->stop_code = 0;
        if (d_ptr->signal_callback)
...
void lite_obs_output::lite_obs_output_stop()
{
    if (!i_output_valid())
        return;
    if (!d_ptr->active && !d_ptr->reconnecting)
        return;
    if (d_ptr->reconnecting) {
        lite_obs_output_force_stop();
        return;
    }
    if (!stopping()) {
        if (d_ptr->signal_callback)
            d_ptr->signal_callback->stopping();
        obs_output_actual_stop(false, os_gettime_ns());
    }
}

void lite_obs_output::lite_obs_output_set_media(std::shared_ptr<video_output> vo, std::shared_ptr<audio_output> ao)
{
    d_ptr->video = vo;
    d_ptr->audio = ao;
}
...
bool lite_obs_output::lite_obs_output_initialize_encoders()
{
    ...
    if (i_has_video()) {
        auto vc = d_ptr->video_encoder.lock();
        if (vc)
            return vc->obs_encoder_initialize();
    ...
    if (i_has_audio()) {
        ...
        if (ac)
            return ac->obs_encoder_initialize();
...
void lite_obs_output::interleave_packets_internal(std::shared_ptr<struct encoder_packet> packet)
{
    if (!d_ptr->active)
        return;
...
void lite_obs_output::interleave_packets(void *data, std::shared_ptr<struct encoder_packet> packet)
{
    auto output = (lite_obs_output *)data;
    output->interleave_packets_internal(packet);
}

void lite_obs_output::default_encoded_callback_internal(std::shared_ptr<struct encoder_packet> packet)
{
...
void lite_obs_output::default_encoded_callback(void *param, std::shared_ptr<struct encoder_packet> packet)
{
...
void lite_obs_output::hook_data_capture(bool encoded, bool has_video, bool has_audio)
{
    ...
    if (encoded) {
        ...
        if (has_video) {
            auto info = obs.obs_core_video()->lite_obs_core_video_info();
        ...
        if (has_video) {
            obs.obs_core_video()->lite_obs_core_video_change_raw_active(true);
            auto vo = d_ptr->video.lock();
            if (vo)
...
{
    lite_obs_core_video *video = obs.obs_core_video();

    uint32_t drawn = video->total_frames() - d_ptr->starting_drawn_count;
    uint32_t lagged = video->lagged_frames() - d_ptr->starting_lagged_count;
...
void lite_obs_output::end_data_capture_thread_internal()
{
    ...
    if (i_encoded()) {
        ...
        if (i_has_video()) {
            obs.obs_core_video()->lite_obs_core_video_change_raw_active(false);
            auto vo = d_ptr->video.lock();
            if (vo)
    ...
    d_ptr->active = false;
    os_event_signal(d_ptr->stopping_event);
    d_ptr->end_data_capture_thread_active =false;
}
...
void lite_obs_output::lite_obs_output_end_data_capture_internal(bool sig)
{
    ...
    d_ptr->end_data_capture_thread_active = true;
